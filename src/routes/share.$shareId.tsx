    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto flex max-w-xl flex-col items-center px-5 py-28 text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          It may have expired or the address may be incomplete.
        </p>
        <Button asChild className="mt-7">
          <Link to="/">Back to ATS Resume Ready home</Link>
        </Button>
      </main>
      <SiteFooter />
    </div>
...
export const Route = createFileRoute("/share/$shareId")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      {
        title: loaderData
          ? `${loaderData.job_title} at ${loaderData.company} | Feedback Preview`
          : "Feedback | ATS Resume Ready",
      },
      {
        name: "description",
        content: loaderData
          ? `Review a tailored application for ${loaderData.job_title} at ${loaderData.company}.`
          : "Review a shared ATS Resume Ready application.",
      },
      {
        property: "og:title",
        content: loaderData
      ...
      },
      {
        ...
        content: "A job-specific resume preview shared for trusted feedback.",
      },
        ...
        <section className="rounded-3xl border border-border bg-card p-6 shadow-card sm:p-8">
          ...
          <div className="mt-3 flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-lg text-muted-foreground">{share.company}</p>
              {share.location && (
            ...
            {share.job_url && (
              <Button asChild variant="outline">
                <a href={share.job_url} target="_blank" rel="noopener noreferrer">
          ...
          <p className="mt-6 border-t border-border pt-5 text-xs text-muted-foreground">
            Shared {new Date(share.created_at).toLocaleDateString()} · Expires{" "}
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
          <aside className="space-y-4">
            <div className="rounded-3xl border border-border bg-card p-5 shadow-card">
              <h2 className="text-sm font-semibold text-foreground">Application signals</h2>
            ...
            <div className="rounded-3xl border border-border bg-card p-5 shadow-card">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {result.match.missingKeywords.slice(0, 12).map((keyword: string) => (
                  <span
                    key={keyword}
          ...
          <article className="rounded-3xl border border-border bg-card p-4 shadow-card sm:p-6">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-muted-foreground">{result.resume.summary}</p>
            </div>
            <div
              className="latex-preview max-h-[900px] overflow-auto rounded-xl border border-border bg-card p-4 sm:p-6"
              dangerouslySetInnerHTML={{ __html: renderResumeHtml(result.resume.latex) }}
            />
          </article>
        </section>
      </main>
